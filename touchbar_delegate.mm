...
- (BOOL)respondsToSelector: (SEL)selector
{
    ...
    return [super respondsToSelector: selector] || [_qtDelegate respondsToSelector: selector];
}
...
- (void)uninstallDelegate
{
    ...
    id const delegate = [self.target valueForKey: @"delegate"];
...
{
    auto view = (__bridge NSView*)reinterpret_cast<void*>(window->winId()); // ARC-allowed cast
